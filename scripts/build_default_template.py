...
def build(output: Path):
    ...
    prompt(doc, "模板使用说明：替换灰色提示与示例行；未知元数据标为待确认。按需删除可选内容并更新编号、交叉引用与目录。")
    ...
    prompt(doc, "说明约束、方案、备选项、选择理由和后果。并发、状态、实时性及部署内容按需融入本章或模块章节，不默认单列。")
