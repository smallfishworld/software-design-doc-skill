...
- 需求先按用户能力 / 功能流程拆解，再映射到模块和代码，不按源码目录反推需求章节
- 用户已手工修改 Word 时采用 DOCX-first 增量编辑，避免旧 Markdown 覆盖已定稿内容
...
8. 多任务与并发设计（条件候选章节）
9. 状态行为设计（条件候选章节）
10. 性能、实时性与资源设计（条件候选章节）
11. 部署、升级与兼容性设计（条件候选章节）
12. 异常与故障处理
13. 日志与可观测性
14. 安全设计（项目相关时）
15. 可测试性设计
...
对四个候选章节分别采用三级判定：影响全局架构时保留为独立章节；只影响局部模块时融合到总体架构、模块、数据、接口、故障或风险章节；无关时删除，不保留空章节。常规编译命令、打包步骤和部署操作不属于概要设计；只有工具链、ABI、交叉编译、部署拓扑、升级和兼容性确实约束架构时才纳入。
...
- 正式 UML 图优先 PlantUML；分层逻辑架构、框图、流程图、树形图、功能分解和通用关系图优先 Mermaid。工具、渲染和格式降级规则维护在 [绘图指南](references/diagram-guide.md)。
